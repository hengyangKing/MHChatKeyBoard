...
+ (instancetype)keyBoardWithStyle:(BarStyle)style
{
    return [self keyBoardWithNavgationBarTranslucent:YES WithStyle:style];
}
 ...
 */
+ (instancetype)keyBoardWithNavgationBarTranslucent:(BOOL)translucent WithStyle:(BarStyle)style
{
    ...
    return [[self alloc] initWithFrame:frame withStyle:style];
}

+ (instancetype)keyBoardWithParentViewBounds:(CGRect)bounds WithStyle:(BarStyle)style
{
...
- (instancetype)initWithFrame:(CGRect)frame withStyle:(BarStyle)barStyle
{
    ...
    if (self)
    {
        _chatToolBar = [[ChatToolBar alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kChatToolBarHeight)withStyle:barStyle];
...
-(void)chatToolBarPointUser:(NSString *)text
{
    if ([self.delegate respondsToSelector:@selector(chatToolBarPointUser:)]) {
        [self.delegate chatToolBarPointUser:text];
    }
}
- (void)chatToolBarAddTopic:(NSString *)text
{
    if ([self.delegate respondsToSelector:@selector(chatToolBarAddTopic:)]) {
        [self.delegate chatToolBarAddTopic:text];
    }
}
